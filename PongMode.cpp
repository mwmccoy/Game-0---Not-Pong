...
PongMode::PongMode() {
	...
	{
		pongball->ball_trail.clear();
		pongball->ball_trail.emplace_back(pongball->ball, pongball->trail_length);
		pongball->ball_trail.emplace_back(pongball->ball, 0.0f);
	}
...
void PongMode::update(float elapsed) {
	...
	if (balls.size() > 0)
	{
		...
		{
			if ((balls[i].ball.x > aiBall.ball.x && balls[i].ball_velocity.x > 0) || (aiBall.ball_velocity.x < 0))
			{
				aiBall = balls[i];
			}
		}
		...
		{ //right player ai:
			...
			if (ai_offset_update < elapsed) {
				...
				ai_offset_update = .1f;// (mt() / float(mt.max())) * 0.5f + 0.5f;
				ai_offset = (mt() / float(mt.max())) * 1.0f - 0.5f;
			}
			if (right_paddle.y < aiBall.ball.y + ai_offset) {
				right_paddle.y = std::min(aiBall.ball.y + ai_offset, right_paddle.y + 25.0f * elapsed);
			}
			else {
				right_paddle.y = std::max(aiBall.ball.y + ai_offset, right_paddle.y - 25.0f * elapsed);
			}
		}
	}
	...
	for (auto pongball = balls.begin(); pongball != balls.end(); pongball++)
	{
		pongball->ball += elapsed * speed_multiplier * pongball->ball_velocity;
	}

	//---- collision handling ----

	//paddles:
	auto paddle_vs_ball = [this](PongBall *pongball, glm::vec2 const &paddle) {
		//compute area of overlap:
		glm::vec2 min = glm::max(paddle - paddle_radius, pongball->ball - pongball->ball_radius);
		glm::vec2 max = glm::min(paddle + paddle_radius, pongball->ball + pongball->ball_radius);
		...
		if (max.x - min.x > max.y - min.y) {
			...
			if (pongball->ball.y > paddle.y) {
				pongball->ball.y = paddle.y + paddle_radius.y + pongball->ball_radius.y;
				pongball->ball_velocity.y = std::abs(pongball->ball_velocity.y);
			} else {
				pongball->ball.y = paddle.y - paddle_radius.y - pongball->ball_radius.y;
				pongball->ball_velocity.y = -std::abs(pongball->ball_velocity.y);
			}
			...
			if (pongball->ball.x > paddle.x) {
				pongball->ball.x = paddle.x + paddle_radius.x + pongball->ball_radius.x;
				pongball->ball_velocity.x = std::abs(pongball->ball_velocity.x);
			} else {
				pongball->ball.x = paddle.x - paddle_radius.x - pongball->ball_radius.x;
				pongball->ball_velocity.x = -std::abs(pongball->ball_velocity.x);
			}
			//warp y velocity based on offset from paddle center:
			float vel = (pongball->ball.y - paddle.y) / (paddle_radius.y + pongball->ball_radius.y);
			pongball->ball_velocity.y = glm::mix(pongball->ball_velocity.y, vel, 0.75f);
		}
	};
	...
	for (auto pongball = balls.begin(); pongball != balls.end(); pongball++)
	{
		paddle_vs_ball(&(*pongball), left_paddle);
		paddle_vs_ball(&(*pongball), right_paddle);
	}

	for (auto pongball = balls.begin(); pongball != balls.end(); pongball++)
	{
		//court walls:
		if (pongball->ball.y > court_radius.y - pongball->ball_radius.y) {
			pongball->ball.y = court_radius.y - pongball->ball_radius.y;
			if (pongball->ball_velocity.y > 0.0f) {
				pongball->ball_velocity.y = -pongball->ball_velocity.y;
			}
		}
		if (pongball->ball.y < -court_radius.y + pongball->ball_radius.y) {
			pongball->ball.y = -court_radius.y + pongball->ball_radius.y;
		...
		if (pongball->ball.x > court_radius.x - pongball->ball_radius.x) {
			pongball->ball.x = court_radius.x - pongball->ball_radius.x;
			if (pongball->ball_velocity.x > 0.0f) {
				pongball->ball_velocity.x = -pongball->ball_velocity.x;
				pongball->should_delete = true;
				left_score += 1;
			}
		}
		if (pongball->ball.x < -court_radius.x + pongball->ball_radius.x) {
		...
		for (auto& t : pongball->ball_trail) {
			t.z += elapsed;
		}
		...
		while (pongball->ball_trail.size() >= 2 && pongball->ball_trail[1].z > pongball->trail_length) {
			pongball->ball_trail.pop_front();
		}
	}

	for (auto pongball = balls.begin(); pongball != balls.end(); pongball++)
	{
		if (pongball->should_delete)
		{
			balls.erase(pongball--);
		}
	}
}
...
void PongMode::draw(glm::uvec2 const &drawable_size) {
	...
	for (auto pongball = balls.begin(); pongball != balls.end(); pongball++)
	{
		draw_rectangle(pongball->ball + s, pongball->ball_radius, shadow_color);

		//ball's trail:
		if (pongball->ball_trail.size() >= 2) {
			//start ti at second element so there is always something before it to interpolate from:
			std::deque< glm::vec3 >::iterator ti = pongball->ball_trail.begin() + 1;
			//draw trail from oldest-to-newest:
			constexpr uint32_t STEPS = 20;
			//draw from [STEPS, ..., 1]:
			for (uint32_t step = STEPS; step > 0; --step) {
				//time at which to draw the trail element:
				float t = step / float(STEPS) * pongball->trail_length;
				//advance ti until 'just before' t:
				while (ti != pongball->ball_trail.end() && ti->z > t) ++ti;
				//if we ran out of recorded tail, stop drawing:
				if (ti == pongball->ball_trail.end()) break;
				...
				draw_rectangle(at, pongball->ball_radius, color);
			}
		}
	}
	...
	for (auto pongball = balls.begin(); pongball != balls.end(); pongball++)
	{
		draw_rectangle(pongball->ball, pongball->ball_radius, fg_color);
	}
